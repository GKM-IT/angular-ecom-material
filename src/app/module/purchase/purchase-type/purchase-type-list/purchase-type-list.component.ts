...
@Component({
  ...
  styleUrls: ['./purchase-type-list.component.css']
})
export class PurchaseTypeListComponent implements OnInit {
  ...
  constructor(
    private masterService: PurchaseTypeService,
  ...
  edit(row) {
    this.router.navigate(['/purchase-type/', row.id]);
  }

  add() {
    this.router.navigate(['/purchase-type/', 'new']);
  }
