...
export class TaxRateListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['select', 'name', 'rate', 'type', 'tax_class', 'updated_at', 'action'];
  ...
  constructor(
    private masterService: TaxRateService,
  ...
  edit(row) {
    this.router.navigate(['/tax-rate/', row.id]);
  }

  add() {
    this.router.navigate(['/tax-rate/', 'new']);
  }
