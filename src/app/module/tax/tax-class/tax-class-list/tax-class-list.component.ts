...
@Component({
  selector: 'app-tax-class-list',
  templateUrl: './tax-class-list.component.html',
  styleUrls: ['./tax-class-list.component.css']
})
export class TaxClassListComponent implements OnInit {

  displayedColumns: string[] = ['select', 'name', 'sort_order', 'updated_at', 'action'];
  ...
  constructor(
    private masterService: TaxClassService,
  ...
  edit(row) {
    this.router.navigate(['/tax-class/', row.id]);
  }

  add() {
    this.router.navigate(['/tax-class/', 'new']);
  }
}
