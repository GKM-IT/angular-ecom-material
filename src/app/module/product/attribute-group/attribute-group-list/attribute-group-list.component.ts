...
@Component({
  ...
  styleUrls: ['./attribute-group-list.component.css']
})
export class AttributeGroupListComponent implements OnInit {
  ...
  constructor(
    private masterService: AttributeGroupService,
  ...
  edit(row) {
    this.router.navigate(['/attribute-group/', row.id]);
  }

  add() {
    this.router.navigate(['/attribute-group/', 'new']);
  }
