...
export class AttributeGroupService {
  ...
  public list(data: any) {
    ...
    this.url = `${environment.url}product/attribute_groups`;
  ...
  public detail(id: any) {
    ...
    this.url = `${environment.url}product/attribute_groups/detail`;
  ...
  public delete(id: any) {
    this.url = `${environment.url}product/attribute_groups/delete/${id}`;
  ...
  public deleteAll(data: any) {
    ...
    this.url = `${environment.url}product/attribute_groups/delete_all`;
    return this.http.post<any>(this.url, this.formData).pipe(
      // retry(1), // retry a failed request up to 3 times
      catchError(this.configService.handleError)
    );
  ...
  public save(data: any, id: any) {
    this.formData = new FormData();
    this.url = `${environment.url}product/attribute_groups/save`;
    if (id !== 'new') {
      this.formData.append('id', id);
    }
