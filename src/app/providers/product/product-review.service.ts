...
export class ProductReviewService {
  ...
  public list(data: any) {
    ...
    this.url = `${environment.url}product/product_reviews`;
  ...
  public detail(id: any) {
    ...
    this.url = `${environment.url}product/product_reviews/detail`;
  ...
  public delete(id: any) {
    this.url = `${environment.url}product/product_reviews/delete/${id}`;
  ...
  public deleteAll(data: any) {
    ...
    this.url = `${environment.url}product/product_reviews/delete_all`;
    return this.http.post<any>(this.url, this.formData).pipe(
      // retry(1), // retry a failed request up to 3 times
      catchError(this.configService.handleError)
    );
  ...
  public save(data: any, id: any) {
    this.formData = new FormData();
    this.url = `${environment.url}product/product_reviews/save`;
    if (id !== 'new') {
      this.formData.append('id', id);
    }
