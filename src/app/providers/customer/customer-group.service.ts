...
export class CustomerGroupService {
  ...
  public list(data: any) {
    ...
    this.url = `${environment.url}customer/customer_groups`;
  ...
  public detail(id: any) {
    ...
    this.url = `${environment.url}customer/customer_groups/detail`;
  ...
  public delete(id: any) {
    this.url = `${environment.url}customer/customer_groups/delete/${id}`;
  ...
  public deleteAll(data: any) {
    ...
    this.url = `${environment.url}customer/customer_groups/delete_all`;
    return this.http.post<any>(this.url, this.formData).pipe(
      // retry(1), // retry a failed request up to 3 times
      catchError(this.configService.handleError)
    );
  ...
  public save(data: any, id: any) {
    this.formData = new FormData();
    this.url = `${environment.url}customer/customer_groups/save`;
    if (id) {
      this.formData.append('id', id);
    }
